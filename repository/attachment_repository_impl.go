...
func (repository *AttachmentRepositoryImpl) Save(ctx context.Context, tx *sql.Tx, attachment domain.Attachment) domain.Attachment {
	SQL := "insert into attachments(todo_id, location, caption, created_at, updated_at) values (?, ?, ?, ?, ?)"
	result, err := tx.ExecContext(ctx, SQL, attachment.TodoId, attachment.Location, attachment.Caption, time.Now(), time.Now())
...
func (repository *AttachmentRepositoryImpl) Update(ctx context.Context, tx *sql.Tx, attachment domain.Attachment) domain.Attachment {
	SQL := "update attachments set todo_id = ?, location = ?, caption = ?, updated_at = ? where id = ?"
	_, err := tx.ExecContext(ctx, SQL, attachment.TodoId, attachment.Location, attachment.Caption, time.Now(), attachment.Id)
	helper.PanicIfError(err)
...
func (repository *AttachmentRepositoryImpl) FindById(ctx context.Context, tx *sql.Tx, attachmentId int) (domain.Attachment, error) {
	SQL := "select id, todo_id, location, caption from attachments where id = ?"
	...
	if rows.Next() {
		err := rows.Scan(&attachment.Id, &attachment.TodoId, &attachment.Location, &attachment.Caption)
		helper.PanicIfError(err)
...
func (repository *AttachmentRepositoryImpl) FindAll(ctx context.Context, tx *sql.Tx) []domain.Attachment {
	SQL := "select id, todo_id, location, caption from attachments"
